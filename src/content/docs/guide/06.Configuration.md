...
| `siteUrl` | string | required | Canonical base URL for your site, used for canonical links and Open Graph URLs. Must start with `https://` or `http://`. |
...
Extra icon buttons rendered in the navbar between the search bar and the theme toggle. The `icon` field accepts any name from the [Icons reference](/reference/icons). External links that start with `http` open in a new tab automatically. Omit `icon` to render the label as plain text.
